...
RUN apt-get update
RUN apt-get install -y zlib1g-dev git wget libncurses5-dev unzip python python-pip libbz2-dev liblzma-dev pkg-config automake libglib2.0-dev default-jre software-properties-common bowtie2 bedtools tabix vcftools
RUN apt-get clean
...
RUN cd $HOME/bin && wget http://gallifrey.thruhere.net/vcfsorter.pl && chmod a+x vcfsorter.pl
